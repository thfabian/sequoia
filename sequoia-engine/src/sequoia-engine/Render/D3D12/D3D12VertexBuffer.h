...
#ifndef SEQUOIA_ENGINE_RENDER_D3D12_D3D12VERTEXBUFFER_H
#define SEQUOIA_ENGINE_RENDER_D3D12_D3D12VERTEXBUFFER_H
...
/// @brief D3D12 buffer for storing vertices
///
/// This is simply a HostBuffer.
///
/// @ingroup gl
class SEQUOIA_API D3D12VertexBuffer final : public VertexBuffer {
  std::unique_ptr<core::HostBuffer> buffer_; ///< Mock data

public:
  D3D12VertexBuffer(const VertexLayout& layout);

  /// @brief Free all memory
  ~D3D12VertexBuffer();

  static bool classof(const Buffer* buffer) { return buffer->getKind() == BK_D3D12VertexBuffer; }
